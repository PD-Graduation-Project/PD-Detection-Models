...
class StatisticalFeatureExtractor(nn.Module):
    ...
    Input:  x: [B, C, T] where C=3 (accel_xyz)
    Output: [B, out_dim] feature vector
    """
    def __init__(self, out_dim=32):
        super().__init__()
        self.proj = nn.Sequential(
            nn.Linear(7, 64), # 7 statistical features (from accelerometer) -> embed_dim = 64
            nn.LayerNorm(64),
    ...
    def forward(self, x):
        ...
        # Compute 7 statistical moments for accelerometer signal
        stats = self._compute_stats(x)  # [B, 7]
        
        return self.proj(stats) # Output: [B, 7] -> [B, out_dim]
...
class FrequencyAnalyzer(nn.Module):
    ...
    Input:  left_raw, right_raw: [B, C, T] where C=3 (accel only), T=1024 samples (~10 sec at 100Hz)
