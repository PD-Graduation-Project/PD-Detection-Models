...
class TremorNetV9(nn.Module):
    def __init__(self, dropout=0.45, all_movements=False, num_movements=11):
        ...
        self.metadata_encoder = ClinicalMetadataEncoder(
            output_dim=96,
            dropout=dropout
        )
        
        # === METADATA-SIGNAL CROSS-ATTENTION ===
        # Allow metadata to modulate signal features
        self.signal_metadata_attn = nn.MultiheadAttention(
            embed_dim=128,
            num_heads=4,
            dropout=dropout * 0.5,
            batch_first=True
        ...
        self.fusion = nn.Sequential(
            ...
            # Expansion with metadata-aware path
            nn.Linear(192, 320),
            nn.BatchNorm1d(320),
            nn.GELU(),
    ...
    def forward(self, x, handedness, metadata, movements=None, mixup_lambda=None):
        ...
        Args:
            x: [B, 2, T, 6]
            handedness: [B]
            metadata: [B, 8] - clinical metadata vector
        ...
        if mixup_lambda is not None and self.training:
            ...
            metadata = mixup_lambda * metadata + (1 - mixup_lambda) * metadata[indices]
        ...
        metadata_feat = self.metadata_encoder(metadata)  # [B, 96]
        ...
        metadata_query = metadata_feat.unsqueeze(1)  # [B, 1, 96]
        # Project metadata to match signal dimension
        metadata_proj = F.linear(metadata_query, 
                                 torch.randn(128, 96, device=metadata_query.device) * 0.02)  # [B, 1, 128]
        
        # Cross-attention: metadata queries signal features
        modulated_signal, _ = self.signal_metadata_attn(
            metadata_proj,  # query
            signal_stack,   # key
            signal_stack    # value
        )
        modulated_signal = modulated_signal.squeeze(1)  # [B, 128]
        ...
        features_list = [
            ...
            bilateral_feat,
            hand_emb,
            metadata_feat  # NEW: clinical metadata
        ]
