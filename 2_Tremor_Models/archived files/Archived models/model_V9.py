"""
TremorNetV9 - Enhanced tremor detection network
...
class TremorNetV9(nn.Module):
    """
    TremorNetV9 with dominant hand pathway and multi-scale feature extraction.
    ...
    def __init__(self, dropout=0.45, all_movements=False, num_movements=11):
        ...
        # ------------- multi-scale time-domain convs (3 channels input) -------------
        self.conv_fast = nn.Conv1d(3, 64, 3, stride=2, padding=1)
        self.conv_mid  = nn.Conv1d(3, 64, 7, stride=2, padding=3)
        self.conv_slow = nn.Conv1d(3, 64, 15, stride=2, padding=7)
        ...
        # ------------- dominant-hand super-pathway (3x capacity, 3 channels) -------------
        self.dom_conv1 = nn.Conv1d(3, 96, 7, stride=2, padding=3); self.dom_bn1 = nn.BatchNorm1d(96)
        ...
        # Temporal attention for frequency features [B, 11, 128] -> [B, 128]
        ...
        # ------------- fusion/classifier (without metadata) -------------
        non_dom_dim, dom_dim, freq_dim = 128, 256, 128
        stat_dim, diff_dim, bilateral_dim, hand_dim = 32, 64, 64, 48
    ...
    # -------------------- helpers --------------------
    def _mixup_apply(self, B, x, handedness, movements, mixup_lambda):
        """Return (x, hand_emb_weights, movements_onehot) after mixup (if applied)."""
        ...
        return x, hand_mix, move_mix

    def _ms_features(self, x):
        """Multi-scale CNN → SE → temporal attention → aggregated [B,128]. x: [B,3,T]"""
    ...
    def forward(self, x, handedness, movements=None, mixup_lambda=None):
        """
        x: [B, 2, T, 3]; handedness: [B]; movements: [B] (optional)
        """
        B = x.shape[0]

        # apply mixup (if requested)
        x, handedness_mix, movements_mix = self._mixup_apply(B, x, handedness, movements, mixup_lambda)

        # left/right raw signals as [B,3,T]
        ...
        # assemble features
        feat_list = [non_dom_feat, dom_feat, freq_feat, stat_feat, diff_feat, bilateral_feat, hand_emb]
        if self.all_movements:
